...
# Descripción:
# Genera el histórico diario de 2023 a partir del archivo especificado,
# exporta CSV y Parquet en data/clean y el reporte de huecos en data/reports.
...
log = logging.getLogger("generar_historico_2023")
...
# Exponer funciones del módulo base (no tocamos la genérica)
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Generar histórico diario de 2023.")
    p.add_argument(
        "--input",
        type=str,
        # Usamos el archivo que me pasaste como default
        default=str(RAW_DIR / "Historico_Ventas_2024_Corregido.xlsx"),
        help="Ruta al archivo de entrada (xlsx/csv) para construir el histórico 2023.",
    )
    ...
    p.add_argument(
        "--qty-col",
        type=str,
        default="Sales Quantity",  # <-- si tu columna es 'Demand', cámbialo aquí
...
def main() -> None:
    args = _parse_args()
    print(">>> Iniciando generar_historico_2023.py")
