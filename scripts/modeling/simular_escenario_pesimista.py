...
# Descripción:
#   Genera el escenario PESIMISTA a partir del baseline estacionalizado,
#   usando factores derivados del WAPE medio por clúster (backtesting 8.5).
#
# Flujo del pipeline:
# 1) Carga baseline y métricas (WAPE por clúster)
# 2) Cálculo de factores (pesimista = 1 - WAPE_%/100; acotado a ≥ 0)
...
#
# Output (por defecto):
#   - data/processed/predicciones_2025_pesimista.parquet
#   - outputs/controles_escenarios/control_totales_pesimista.csv
#   - outputs/controles_escenarios/control_por_cluster_pesimista.csv
#
# Dependencias:
#   - pandas
#   - numpy
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Genera el escenario PESIMISTA.")
    ...
    p.add_argument("--out",      type=Path, default=PROCESSED_DIR / "predicciones_2025_pesimista.parquet")
    ...
    p.add_argument("--min-factor", type=float, default=0.0)
    return p.parse_args()
...
def run(args: argparse.Namespace) -> None:
    ...
    log.info("Calculando factores (pesimista = 1 - WAPE_%/100)")
    factors = U["calcular_factores"](metrics, escenario="pesimista",
    ...
    factors.to_csv(args.controles_dir / "factores_pesimista.csv", index=False)

    df_pes = U["aplicar_factores"](df_base, factors, on=[args.cluster_col], yhat_col=args.yhat_col)

    U["validate_structure"](df_pes, date_col=args.date_col, cluster_col=args.cluster_col,
                            yhat_col=args.yhat_col, expect_year=args.year)

    U["ensure_dirs"](args.out.parent)
    df_pes.to_parquet(args.out, index=False)

    U["generar_controles"](df_base, df_pes, args.controles_dir, escenario="pesimista",
                           cluster_col=args.cluster_col, yhat_col=args.yhat_col)
    ...
    meta = {
        ...
        "total_escenario": float(df_pes[args.yhat_col].sum()),
    ...
    (args.controles_dir / "meta_pesimista.json").write_text(json.dumps(meta, indent=2, ensure_ascii=False))
    log.info("✅ Escenario PESIMISTA generado correctamente.")
